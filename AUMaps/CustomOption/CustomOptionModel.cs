...
namespace AUMaps.AUMaps.CustomOption
{
    public class CustomOption
    {
        ...
        public CustomOption(int id, string name, System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, bool isHidden, string format)
        {
            ...
            if (id > 0)
            {
                entry = AUMapsPlugin.Instance.Config.Bind($"Preset{preset}", id.ToString(), defaultSelection);
                selection = Mathf.Clamp(entry.Value, 0, selections.Length - 1);
                if (options.Any(x => x.id == id))
                {
                    AUMapsPlugin.Logger.LogInfo("CustomOptionのId(" + id + ")が重複しています。");
                }
                if (Max < id)
                {
                    Max = id;
        ...
        public static void switchPreset(int newPreset)
        {
            ...
            foreach (CustomOption option in CustomOption.options)
            {
                ...
                option.entry = AUMapsPlugin.Instance.Config.Bind($"Preset{preset}", option.id.ToString(), option.defaultSelection);
    ...
    class GameOptionsMenuStartPatch
    {
        ...
        {
            if (GameObject.Find("AUMSettings") != null)
            { // Settings setup has already been performed, fixing the title of the tab and returning
                GameObject.Find("AUMSettings").transform.FindChild("GameGroup").FindChild("Text").GetComponent<TMPro.TextMeshPro>().SetText(ModTranslation.getString("SettingAUMapsSetting"));
                return;
            }
            ...
            var AUMSettings = UnityEngine.Object.Instantiate(gameSettings, gameSettings.transform.parent);
            var torMenu = AUMSettings.transform.FindChild("GameGroup").FindChild("SliderInner").GetComponent<GameOptionsMenu>();
            AUMSettings.name = "AUMSettings";
            torMenu.name = "AUMapsSlider";
            ...
            torTab.transform.FindChild("Hat Button").FindChild("Icon").GetComponent<SpriteRenderer>().sprite = ModHelpers.loadSpriteFromResources("AUMaps.Resources.CrewMateSetting.png", 100f);            
            ...
            for (int i = 0; i < tabs.Length; i++)
            {
                ...
                button.OnClick.AddListener((UnityEngine.Events.UnityAction)(() => {
                    ...
                    AUMSettings.gameObject.SetActive(false);
                    ...
                    {
                        AUMSettings.gameObject.SetActive(true);
    ...
    class GameOptionsMenuUpdatePatch
    {
        ...
        public static void Postfix(GameOptionsMenu __instance)
        {
            AUMapsPlugin.Logger.LogInfo("名前:"+__instance.name);
            if (__instance.name != "AUMapsSlider") return; // TODO: Introduce a cleaner way to seperate the TOR settings from the game settings
    ...
    class GameOptionsDataPatch
    {
        ...
        public static string ResultData()
        {
            ...
            int counter = AUMapsPlugin.optionsPage = AUMapsPlugin.optionsPage % numPages;
    ...
    public static class GameOptionsNextPagePatch
    {
        public static void Postfix(KeyboardJoystick __instance)
        {
            ...
            {
                AUMapsPlugin.optionsPage = AUMapsPlugin.optionsPage + 1;
            }
        }
    }
